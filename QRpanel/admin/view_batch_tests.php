...
// Bu batch time'daki testleri al
$stmt = $conn->prepare("SELECT * FROM test_results WHERE DATE_FORMAT(created_at, '%Y-%m-%d %H:%i') = ? ORDER BY id ASC");
...
<head>
    ...
    <title><?php echo date('d.m.Y H:i', strtotime($batch_time)); ?> Toplu Oluşturma - CROMTEST</title>
...
<body>
    <div class="admin-container">
        <header class="admin-header">
            <div class="header-content">
                <div class="admin-logo-title">
                    ...
                    <h1>📦 <?php echo date('d.m.Y H:i', strtotime($batch_time)); ?> Toplu Oluşturma</h1>
                </div>
                <div class="user-info">
        ...
        <main class="admin-main">
            ...
            <div class="info-box" style="margin-bottom: 20px; padding: 15px; background: linear-gradient(135deg, #f3e8ff 0%, #e9d5ff 100%); border-radius: 10px; border-left: 4px solid #8b5cf6;">
                <p style="color: #581c87; margin: 0;">
                    <strong>🕐 Oluşturulma Zamanı:</strong> <?php echo date('d.m.Y H:i:s', strtotime($tests[0]['created_at'])); ?>
                    <span style="margin-left: 20px;"><strong>📊 Toplam:</strong> <?php echo $total_count; ?> adet test</span>
                </p>
            </div>
            <?php endif; ?>
            ...
            <div class="stats-grid" style="margin-bottom: 30px;">
                ...
                <div class="stat-card" style="background: linear-gradient(135deg, #f3e8ff 0%, #e9d5ff 100%);">
                    <div class="stat-icon">📥</div>
                    <div class="stat-info">
                        <a href="download_qr_batch.php?batch_time=<?php echo urlencode($batch_time); ?>&csrf_token=<?php echo urlencode(generateCSRFToken()); ?>" 
